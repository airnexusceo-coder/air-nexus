export type MotivationSyncInput = { lifetimePoints: number; currentStreakDays: number; longestStreakDays: number }

const MAX_STREAK_DAYS = 3650
const MAX_LIFETIME_POINTS = 1_000_000
...
export function clampMotivationSyncInput(input: MotivationSyncInput): MotivationSyncInput {
  ...
  const lifetimePoints = Math.max(0, Math.min(MAX_LIFETIME_POINTS, Math.round(Number(input.lifetimePoints) || 0)))
  return { lifetimePoints, currentStreakDays, longestStreakDays }
}
