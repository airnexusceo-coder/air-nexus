...
type MessageRow = { id: string; room_id: string; sender_id: string; body: string; created_at: string }
 ...
 * recent conversation would never appear on open.
 */
export async function listRoomMessages(auth: ServerAuthSession, roomId: string, sinceIso?: string | null): Promise<RoomMessageDTO[]> {
  ...
  const response = await supabaseRestFetch(
    auth.accessToken,
    `/room_messages?room_id=eq.${encode(roomId)}&select=id,room_id,sender_id,body,created_at${query}`,
  )
...
export async function sendRoomMessage(auth: ServerAuthSession, roomId: string, body: string): Promise<RoomMessageDTO> {
  ...
  const response = await supabaseRestFetch(auth.accessToken, '/room_messages', {
    ...
    body: JSON.stringify({ room_id: roomId, sender_id: auth.user.id, body: trimmed }),
  })
