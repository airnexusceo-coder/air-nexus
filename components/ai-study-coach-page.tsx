...
export function AiStudyCoachPage({ profileName, transactions, onNavigate, notify }: AiStudyCoachPageProps) {
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[2rem] border border-violet-300/15 bg-[radial-gradient(circle_at_top_left,rgba(139,92,246,0.2),transparent_42%),linear-gradient(135deg,rgba(15,23,42,0.96),rgba(2,6,23,0.99))] p-6 sm:p-8">
        <div className="absolute right-10 top-8 size-32 rounded-full bg-violet-500/10 blur-3xl" />
        <div className="relative flex flex-wrap items-start justify-between gap-5">
          <div className="max-w-3xl"><span className="inline-flex items-center gap-2 rounded-full border border-violet-300/20 bg-violet-400/10 px-3 py-1 text-xs font-semibold text-violet-200"><Compass className="size-3.5" />Proactive daily coaching</span><p className="mt-5 text-sm text-slate-400">Good {new Date().getHours() < 12 ? 'morning' : new Date().getHours() < 18 ? 'afternoon' : 'evening'}, {firstName}.</p><h2 className="mt-2 text-3xl font-semibold tracking-tight text-white sm:text-4xl">{currentPlan.headline}</h2><p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300">{currentPlan.motivation}</p></div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[{ label: 'Active days', value: snapshot.activeDaysThisWeek, detail: 'this week', icon: Activity }, { label: 'Study streak', value: snapshot.currentStreak, detail: 'active days', icon: Flame }, { label: 'Study actions', value: snapshot.studyActionsThisWeek, detail: 'this week', icon: TrendingUp }, { label: 'Assignments', value: `${snapshot.averageAssignmentProgress}%`, detail: `${snapshot.assignmentsDueSoon} due soon`, icon: Target }].map((metric) => { const Icon = metric.icon; return <article key={metric.label} className="glass rounded-2xl p-4"><div className="flex items-center justify-between"><span className="text-xs text-slate-500">{metric.label}</span><Icon className="size-4 text-violet-300" /></div><p className="mt-3 text-2xl font-semibold text-white">{metric.value}</p><p className="mt-1 text-xs text-slate-500">{metric.detail}</p></article> })}
      </section>

      <div className="grid gap-5 xl:grid-cols-[1.35fr_0.9fr]">
        <section className="glass rounded-3xl p-5 sm:p-6">
          <div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-2xl bg-violet-400/12 text-violet-200"><BrainCircuit className="size-5" /></span><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-violet-300">Progress analysis</p><h3 className="mt-1 text-lg font-semibold text-white">What the coach sees</h3></div></div>
          <p className="mt-5 text-sm leading-7 text-slate-300">{currentPlan.progressSummary}</p>
          <button type="button" onClick={() => onNavigate('Daily Dashboard')} className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-violet-200 hover:text-white">Open detailed progress <ArrowRight className="size-3.5" /></button>
        </section>
      ...
      <section className="glass rounded-3xl p-5 sm:p-6">
        <div className="flex items-center gap-3"><CalendarClock className="size-5 text-amber-300" /><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-amber-300">Adaptive revision</p><h3 className="mt-1 font-semibold text-white">Changes to make today</h3></div></div>
        <div className="mt-5 grid gap-3 lg:grid-cols-2">{currentPlan.revisionAdjustments.length === 0 ? <p className="text-sm text-slate-500">No existing revision plan was found to adjust.</p> : currentPlan.revisionAdjustments.map((item, index) => <article key={`${item.subject}-${index}`} className="rounded-2xl border border-white/8 bg-white/[0.025] p-4"><p className="text-xs font-semibold uppercase tracking-wider text-amber-300">{item.subject}</p><p className="mt-2 text-sm font-medium text-white">{item.change}</p><p className="mt-2 text-xs leading-5 text-slate-500">{item.reason}</p></article>)}</div>
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
        ...
        <div className="rounded-3xl border border-violet-300/15 bg-violet-400/8 p-5 sm:p-6"><div className="flex items-center gap-3"><Compass className="size-5 text-violet-200" /><h3 className="font-semibold text-white">Coach’s nudge</h3></div><p className="mt-4 text-lg leading-8 text-violet-50">{currentPlan.motivation}</p><button type="button" onClick={() => onNavigate('AI Tutor')} className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-violet-200 hover:text-white">Begin the first session <ArrowRight className="size-3.5" /></button></div>
      </section>
    </div>
  )
}
