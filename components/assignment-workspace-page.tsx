    ...
    <section id={id} className="glass scroll-mt-24 rounded-3xl p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className={cn('flex size-11 items-center justify-center rounded-2xl', complete ? 'bg-emerald-400/12 text-emerald-200' : 'bg-orange-400/12 text-orange-200')}><Icon className="size-5" /></span>
...
${assignment.draft.slice(0, 6_000)}`
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[2rem] border border-orange-300/15 bg-[radial-gradient(circle_at_top_left,rgba(249,115,22,0.18),transparent_42%),linear-gradient(135deg,rgba(15,23,42,0.94),rgba(2,6,23,0.98))] p-6 sm:p-8">
        <div className="absolute right-8 top-7 size-28 rounded-full bg-orange-500/10 blur-3xl" />
        <div className="relative flex flex-wrap items-end justify-between gap-5">
          <div><span className="inline-flex items-center gap-2 rounded-full border border-orange-300/20 bg-orange-400/10 px-3 py-1 text-xs font-semibold text-orange-200"><ClipboardList className="size-3.5" />One assignment, one workspace</span><h2 className="mt-4 text-3xl font-semibold tracking-tight text-white sm:text-4xl">Assignment Workspace</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Turn a brief into a practical checklist, timeline, research notes, draft, references, improvements, and final review—without losing the thread.</p></div>
          {active && <div className="min-w-52 rounded-2xl border border-white/10 bg-white/[0.045] p-4"><div className="flex items-end justify-between"><span className="text-xs text-slate-400">Overall progress</span><strong className="text-2xl text-white">{progress}%</strong></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-gradient-to-r from-orange-500 to-amber-300 transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-2 text-xs text-slate-500">{completedStages} of {STAGES.length} stages complete</p></div>}
        </div>
      </section>
        ...
        <aside className="glass rounded-3xl p-4 lg:sticky lg:top-24">
          ...
          <div className="mt-2 space-y-2">
            ...
            {assignments.map((assignment) => {
              ...
              return <button key={assignment.id} type="button" onClick={() => { setActiveId(assignment.id); setCreating(false); setAiError('') }} className={cn('w-full rounded-2xl border p-3 text-left transition', activeId === assignment.id && !creating ? 'border-orange-300/30 bg-orange-400/10' : 'border-transparent bg-white/[0.035] hover:border-white/10 hover:bg-white/[0.055]')}><p className="truncate text-sm font-medium text-white">{assignment.title}</p><p className="mt-1 truncate text-xs text-slate-500">{assignment.subject} · {formatDueDate(assignment.dueDate)}</p><div className="mt-3 h-1 overflow-hidden rounded-full bg-white/8"><div className="h-full bg-orange-400" style={{ width: `${Math.round((done / STAGES.length) * 100)}%` }} /></div></button>
            })}
          </div>
        </aside>
        ...
        <div className="min-w-0 space-y-5">
          {creating && (
            <section className="glass rounded-3xl p-5 sm:p-7">
              <div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-2xl bg-orange-400/12 text-orange-200"><Plus className="size-5" /></span><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-orange-300">New workspace</p><h3 className="mt-1 text-xl font-semibold text-white">Create an assignment</h3></div></div>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <label className="text-sm text-slate-300">Assignment title<input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="e.g. Biology investigation report" className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2.5 text-white outline-none focus:border-orange-300/40" /></label>
                <label className="text-sm text-slate-300">Subject<input value={form.subject} onChange={(event) => setForm((current) => ({ ...current, subject: event.target.value }))} placeholder="e.g. Biology" className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2.5 text-white outline-none focus:border-orange-300/40" /></label>
                <label className="text-sm text-slate-300">Due date<input type="date" value={form.dueDate} onChange={(event) => setForm((current) => ({ ...current, dueDate: event.target.value }))} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2.5 text-white outline-none focus:border-orange-300/40" /></label>
                <label className="text-sm text-slate-300">Target word count<input type="number" min="100" max="5000" value={form.targetWordCount} onChange={(event) => setForm((current) => ({ ...current, targetWordCount: event.target.value }))} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2.5 text-white outline-none focus:border-orange-300/40" /></label>
              </div>
              <label className="mt-4 block text-sm text-slate-300">Assignment brief<textarea value={form.brief} onChange={(event) => setForm((current) => ({ ...current, brief: event.target.value.slice(0, MAX_BRIEF_LENGTH) }))} placeholder="Paste the task sheet, rubric, question, required structure, and anything your teacher expects…" className="mt-2 min-h-36 w-full resize-y rounded-2xl border border-white/10 bg-slate-950/55 p-4 text-sm leading-6 text-white outline-none focus:border-orange-300/40" /></label>
              <label className="mt-4 block text-sm text-slate-300">Source notes <span className="text-slate-500">(optional)</span><textarea value={form.sourceNotes} onChange={(event) => setForm((current) => ({ ...current, sourceNotes: event.target.value.slice(0, MAX_SOURCE_LENGTH) }))} placeholder="Paste class notes, source details, quotations, or links you have already collected. AirGPT will not invent references." className="mt-2 min-h-28 w-full resize-y rounded-2xl border border-white/10 bg-slate-950/55 p-4 text-sm leading-6 text-white outline-none focus:border-orange-300/40" /></label>
          ...
          {!creating && active && (
            <>
              ...
              <AssignmentSection id="assignment-draft" title="Working draft" eyebrow="Stage 4" icon={PenLine} complete={isStageComplete(active, 'draft')} action={active.draft && <button type="button" onClick={() => toggleStageDone('draft')} className="secondary-action text-xs">{active.stageDone.draft ? 'Reopen' : 'Mark complete'}</button>}>
                <textarea aria-label="Assignment draft" value={active.draft} onChange={(event) => updateAssignment(active.id, (current) => ({ ...current, draft: event.target.value.slice(0, MAX_DRAFT_LENGTH), stageDone: { ...current.stageDone, draft: false } }))} placeholder="The AI-generated draft will appear here. Edit it in your own voice before final review." className="min-h-[420px] w-full resize-y rounded-2xl border border-white/8 bg-slate-950/45 p-5 text-sm leading-7 text-slate-200 outline-none focus:border-orange-300/35" />
