  ...
  return (
    <nav aria-label="Mobile navigation" className={cn('mobile-bottom-nav fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-slate-950/90 px-2 pb-[env(safe-area-inset-bottom)] backdrop-blur-2xl transition-transform duration-200 lg:hidden', keyboardOpen && 'translate-y-full')}>
      <div className="mx-auto grid h-16 max-w-lg grid-cols-5 items-center">
        {items.map(({ label, icon: Icon, active, action, primary }) => <button key={label} type="button" onClick={action} aria-current={active ? 'page' : undefined} className={cn('mobile-touch relative flex min-h-12 flex-col items-center justify-center gap-1 rounded-2xl text-[10px] font-medium text-slate-500 transition active:scale-95', active && 'text-orange-200', primary && '-mt-5')}><span className={cn('flex size-9 items-center justify-center rounded-2xl', primary && 'size-12 bg-gradient-to-br from-orange-500 to-amber-400 text-white shadow-lg shadow-orange-500/30', active && !primary && 'bg-orange-400/10')}><Icon className={cn('size-5', primary && 'size-5.5')} /></span><span>{label}</span>{active && !primary && <span className="absolute bottom-0 size-1 rounded-full bg-orange-300" />}</button>)}
      </div>
    </nav>
  )
}
