...
import { extractTextFromBuffer, normalizeExtractedText } from '@/lib/documents/extract-server'
...
export async function POST(request: Request) {
  ...
  try {
    const extracted = normalizeExtractedText(await extractTextFromBuffer(file.name, Buffer.from(await file.arrayBuffer())))
    if (!extracted) {
